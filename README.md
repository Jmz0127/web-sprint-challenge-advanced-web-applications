...
A token is a way to authenticate a user by turning a users sensitive credentials into a unique, identifiable, alphanumeric code assigned to that user. The token is then sent to the server as a way to authenticate that the user has a right to access some sort of data that the server sends back. This is after the authenticator confirms that the token received is approved.

2. What steps can you take in your web apps to keep your data secure?

Never, never hardcode tokens within client code. Larger scope, don't put anything sensitive like passwords or the like in client code. Instead, use tokens to communicate and authenticate that a user has rights to be authorized to access sensitive data behind API calls and the like.

3. Describe how web servers work.

Web servers hold data and standby for the go ahead to share information with a client. Web servers have code embedded within them that work once it gets a call from a client. That code enables the server to send back what it required in the request. Web servers need some sort of connection ability to receive calls: whether thats the internet connection or some sort of local hardware to receive and send information.
